...
const OrderDetailsCard = ({ order }) => {
	...
	return (
		...
		>
			...
			<Image
				src={order?.eqID?.eqImage || "https://via.placeholder.com/150" } // Added fallback image
          ...
          <Flex direction="column" gap={2}>
            ...
            <Box mt={2}>
              <Text fontWeight="bold" fontSize="md" >
                ...
                Rented From: <span style={{ fontWeight: "normal" }}>{new Intl.DateTimeFormat("en-GB", { day: "2-digit", month: "long", year: "numeric" }).format(new Date(order?.rentFrom))}</span>
              </Text>
              <Text fontWeight="bold" fontSize="md" >
                Rented To: <span style={{ fontWeight: "normal" }}>{new Intl.DateTimeFormat("en-GB", { day: "2-digit", month: "long", year: "numeric" }).format(new Date(order?.rentTo))}</span>
              </Text>
			  <Text fontWeight="bold" fontSize="md" >
                Return Status: <span style={{ fontWeight: "normal" }}>{order?.returnStatus}</span>
              </Text>
			  {order?.fineValue > 0 && (
				<Text fontWeight="bold" fontSize="md" >
                Fine to Pay : <span style={{ fontWeight: "normal" }}>Rs.{order?.fineValue}</span>
              </Text>
			  )}
...
<Modal isOpen={isOpen} onClose={onClose}>
				...
				<ModalContent>
					...
					<ModalBody>
						<VStack spacing={4}>
							<Text fontSize={'lg'} fontWeight={'medium'}>Enter Today Date</Text>
							<Input
					...
					<ModalFooter>
						<Button
							...
							mr={3}
							size={'sm'}
							onClick={() => handleUpdateReturnStatus(order?._id, updatedReturnStatus)}
						>
							Request a Return
						</Button>
						<Button 
